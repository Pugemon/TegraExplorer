...
LPVERSION_BUGFX := 1
LPVERSION_TEA := 001
LPVERSION := \"$(LPVERSION_MAJOR).$(LPVERSION_MINOR).$(LPVERSION_BUGFX).$(LPVERSION_TEA)\"
...
CUSTOMDEFINES += -DLP_VER_MJ=$(LPVERSION_MAJOR) -DLP_VER_MN=$(LPVERSION_MINOR) -DLP_VER_BF=$(LPVERSION_BUGFX) -DLP_VER_TEA=$(LPVERSION_TEA) -DLP_VER=$(LPVERSION)
...
all: $(OUTPUTDIR)/$(TARGET)_small.bin
	...
	@cp $(CURDIR)/output/TegraExplorer_small.bin /mnt/e/Switch/_ir/tea/bootloader/payloads/TegraExplorer.bin
	@cp $(CURDIR)/output/TegraExplorer_small.bin /mnt/e/Switch/_tea/tea/switch/tea-updater/tea-updater.bin
